...
class NeuralNetworkRegressor(_ArtificialNeuralNetwork):

    def __init__(self, *, name: str, path: str, layers: Tuple[int, ...] = ()):
        """
        This class allows you to create an object containing a neural network regressor defined by a name. The
        ...
        :param name: Name of the Neural Network Regressor. Through this name, a saved model can be loaded
        :param layers: Tuple of neurons for each layer (ex: 1st -> 8 neur. 2nd -> 7 neur., ... -> ....,
                       outp -> 1 neur.)
        """
        ...
        try:
            ...
            if layers or len(layers) > 1:
                ...
                self._nn.add(tf.keras.layers.Dense(units=layers[-1]))

                print("RNN successfully created!")
                print(
                    "dimension: {} -> hidden {}; output: {}".format(len(layers[:-1]), layers[:-1], layers[-1]))
                self._nn.compile(optimizer='adam', loss='mean_squared_error', metrics=['mean_squared_error'])
                print("RNN successfully compiled! LossFunction = mean_squared_error")
