...
class BinaryNNClassificator(Ann):
    ...
    def __init__(self, *args, name):
        ...
        if len(args) < 1:
            print("Numero di layers insufficienti. ANN vuota. Inserire almeno 1 hidden layer")
            return

        for arg in args:
            self._nn.add(tf.keras.layers.Dense(units=arg, activation='relu'))

        self._nn.add(tf.keras.layers.Dense(units=1, activation='sigmoid'))

        print("ANN creata con successo!")
        print("dimensione: {} hidden {}; Dimensione output 1".format(len(args), args))
        self._nn.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
        print("ANN compilato con successo: LossFunction = 'binary_crossentropy'")
...
class NNClassificator(Ann):
    ...
    def __init__(self, *args, name):
        ...
        for arg in args[:-1]:
            self._nn.add(tf.keras.layers.Dense(units=arg, activation='relu'))
        self._nn.add(tf.keras.layers.Dense(units=args[-1], activation='softmax'))

        print("ANN creata con successo!")
        print("dimensione: {} hidden {}; Dimensione output: {}".format(len(args[:-1]), args[:-1], args[-1]))
        self._nn.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['accuracy'])
        print("ANN compilato con successo: LossFunction = 'categorical_crossentropy'")
